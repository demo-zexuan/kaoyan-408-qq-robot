...
class RobotState(BaseModel):
    ...
    # IV. 角色扮演相关
    role_play_mode: bool = Field(
        default=False,
        description="是否为角色扮演模式"
    )
    current_role: str = Field(
        default="",
        description="当前角色名称"
    )
    role_settings: dict[str, Any] = Field(
        default_factory=dict,
        description="角色设置"
    )
    ...
    token_usage: int = Field(
        default=0,
        ge=0,
        description="已使用的Token数"
    )
    last_action: str = Field(
        default="",
        description="上一个执行的动作"
    )
...
class Context(BaseModel):
    ...
    state: Optional[RobotState] = Field(
        default=None,
        description="关联的对话状态"
    )
    current_role_id: Optional[str] = Field(
        default=None,
